...
function calcularValorParte(parte) {
    const valorTotal = parte.cantidad * parte.precio_unitario;
    return valorTotal;
}
...
function calcularTotalInventario(listado) {
    ...
    for (let i = 0; i < listado.length; i++) {
        let parte = listado[i];
        total += calcularValorParte(parte);
    }
...
function calcularPorcentajeOcupacion(parte) {
    ...
    let porcentaje = (parte.cantidad / parte.capacidad_maxima) * 100;
...
function renderizarListaPartes(lista) {
    let contenedor = document.getElementById("listaParts");
    contenedor.innerHTML = "";

    let alertaCount = 0;

    lista.forEach(function (parte) {
        let enAlerta = verificarStockMinimo(parte);
        let porcentaje = calcularPorcentajeOcupacion(parte);

        if (enAlerta) alertaCount++;

        let nivelClase = enAlerta
            ? (parte.cantidad === 0 ? "molecule-part-card--critico" : "molecule-part-card--alerta")
            : "molecule-part-card--normal";

        let badgeClase = enAlerta
            ? (parte.cantidad === 0 ? "atom-badge--red" : "atom-badge--orange")
            : "atom-badge--green";

        let badgeTexto = enAlerta
...
function seleccionarParte(parte) {
    ...
    let porcentaje = calcularPorcentajeOcupacion(parte);
    let valor = calcularValorParte(parte);
    let enAlerta = verificarStockMinimo(parte);
    ...
    let estadoEl = document.getElementById("det-estado-stock");
    if (enAlerta === true) {
...
function actualizarValorTotal() {
    let total = calcularTotalInventario(partes);
    let el = document.getElementById("valorTotalInventario");
...
function renderizarLog() {
    let contenedor = document.getElementById("listaLog");
    contenedor.innerHTML = "";
    ...
    let recientes = historialMovimientos.slice().reverse();
...
function procesarMovimiento(tipo) {
    let idParte = document.getElementById("selectParte").value;
    let cantidad = parseInt(document.getElementById("inputCantidad").value, 10);
    let nota = document.getElementById("inputNota").value.trim();
    let msgEl = document.getElementById("mensajeOperacion");
    ...
    let parte = partes.find(function (p) { return p.id === idParte; });
    let resultado = tipo === "entrada"
        ? registrarEntrada(idParte, cantidad)
        : registrarSalida(idParte, cantidad);

    if (resultado === true) {
        let now = new Date();
        let ts = String(now.getUTCHours()).padStart(2, "0") + ":" +
